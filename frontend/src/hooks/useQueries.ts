...
export function useUpdatePaymentLinkStatus() {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['paymentLinks'] });
      queryClient.invalidateQueries({ queryKey: ['leads'] });
      toast.success('Payment link status updated successfully');
    },
    onError: (error: Error) => {
      toast.error(`Failed to update payment link status: ${error.message}`);
    },
  });
}

// Generator Log Queries
export function useGetMyGeneratorLogs(enabled = true) {
  const { actor, isFetching } = useActor();

  return useQuery<GeneratorLog[]>({
    queryKey: ['generatorLogs'],
    queryFn: async () => {
      if (!actor) return [];
...
export function useCreateGeneratorLog() {
  ...
  return useMutation({
    ...
    onError: (error: Error) => {
      toast.error(`Failed to save generator log: ${error.message}`);
    },
  });
}

// Offer Queries
export function useGetAllOffers(enabled = true) {
  const { actor, isFetching } = useActor();
...
// Integration Settings Queries
export function useGetIntegrationSettings(userId: Principal | null) {
  const { actor, isFetching } = useActor();

  return useQuery<IntegrationSettings | null>({
    queryKey: ['integrationSettings', userId?.toString() || 'null'],
    queryFn: async () => {
